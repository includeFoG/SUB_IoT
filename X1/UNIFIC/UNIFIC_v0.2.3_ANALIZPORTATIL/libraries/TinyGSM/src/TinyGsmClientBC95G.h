/*
Se ha intentado que el módulo sea plug and play pero de momento no se ha conseguido. Al conectar el módulo por primera vez con el "eco" realiza una serie de resets
*/
...
public:
  ...
  bool init(const char* pin = NULL) { 
    DBG(GF("###[TINYGSM] TinyGSM Version:"), TINYGSM_VERSION);
    if (!testAT()) {
      return false;
    }
...
//////////////////////////////////////////////////////////////////////////FUNCIONES PARA BC95G////////////////////////////////////////////////////////////////////////
  bool restart() {
	  delay(12000); //delay para dar tiempo al módulo a que envíe la configuración
    if (!testAT()) {
      return false;
    }
	sendAT(GF("+CMEE=1"));
	...
	sendAT(GF("+NBAND=1,3,5,8,20,28")); //MOVISTAR 4,28  movistar futuro: 2,5,7
    if (waitResponse(10000L) != 1) {
      return false;
    }
	sendAT(GF("+NBAND?"));
...
TINY_GSM_MODEM_GET_CSQ()
  ...
  bool isNetworkConnected() {
	waitResponse(10000L); //NECESARIO POR EL TIEMPO QUE TARDA EL MODULO EN ENCENDERSE, AL USARSE EN OTRA OCASION PUEDE MOSTRAR UN UNHANDLED
...
TINY_GSM_MODEM_WAIT_FOR_NETWORK()
  ...
  String resp="";
	...
	if(waitResponse(30000L,resp,"+CEREG:")==1);{
	   resp = stream.readStringUntil('\n'); 
	   Serial.println("respuesta CEREG: "+String(resp));
	}	
	delay(5000); //TESTING
	if(waitResponse(360000L,resp,"+CEREG:")==1);{ //podría llegar a tadar 360000
	   resp = stream.readStringUntil('\n'); 
	   Serial.println("respuesta CEREG: "+String(resp));
	}
	
	sendAT(GF("+NUESTATS"));  // attach to GPRS
    if (waitResponse(180000L) != 1) { //360000L
      return false;
    }
  ...
  //ESTA FUNCION HA SIDO ACTUALIZADA 21/11/2022
  //para comprobar previa conexión al broker si el dispositivo ya se encuentra conectado (si no el comando de connect da error)
  ...
  Serial.println("PASS: "+String(pwd));
  
  uint8_t indice1, indice2, indice3, check0;
  
  uint8_t ind=0;
  String resp="";
  
  sendAT(GF("+CGATT?"));
  if(waitResponse(GF("ERROR"),GF("+CGATT:1"))!=2){
	  return false;
  }
  ...
  sendAT(GF("+CEREG?"));
  ind = waitResponse(10000L,resp,GF("ERROR"),GF("+CEREG:")); 
  if(ind=!2){
	  waitResponse();  
	  return false;
  }
  ...
  resp = stream.readStringUntil('\n'); //obtiene la trama respuesta
	if(waitResponse(10000L)!=1){ // espera el OK
		return false;//ERROR 2: EL MODULO NO DEVUELVE OK
	}
...
//NUEVO FILTRO ACTUALIZACION 21/11
	sendAT(GF("+QMTCONN?"));       
	 
	 check0= waitResponse(10000L,",3" GSM_NL,",2" GSM_NL,",1" GSM_NL,",4" GSM_NL);//check0= waitResponse(20000L,(String(ID)+",3").c_str(),(String(ID)+",2").c_str(),(String(ID)+",1").c_str());
	 DBG("[TINYGSM]: valor de check0:(",check0,")");
	 waitResponse();
	 	 ...
	 	 if(check0!=1)
		{
			...
			{
				case 2:
						DBG("[TINYGSM]: +QMTCONN?2 CONECTANDO CON BROKER");
						break;
				case 3:
						DBG("[TINYGSM]: +QMTCONN?3 INICIALIZANDO BROKER");
						break;
				case 4:
						DBG("[TINYGSM]:+QMTCONN?4 DESCONECTANDO");
						break;
				default:
						DBG("[TINYGSM]: +QMTCONN? desconocido:", check0);
						break;
			}
		}
		else{
			 DBG("[TINYGSM]: CONEXION CON BROKER PREVIAMENTE ESTABLECIDA");
			 return true;
		}
//FIN NUEVO FILTRO ACTUALIZACION 21/11
	
	  	sendAT(GF("+QIDNSCFG=10.50.0.171,10.50.0.170"));     //config dns  
	...
	sendAT(GF("+QMTOPEN=0,\""), host,"\",", port);    //AT+QMTOPEN? +QMTOPEN: 0,"172.19.1.200",12010
    
    waitResponse();
	
		if(waitResponse(75500L,"0,")!=1)
		{
			return false;
		}
		
		indice1 = waitResponse(10000L,"0" GSM_NL,"-1" GSM_NL,"1" GSM_NL,"2" GSM_NL,"3" GSM_NL);
   ...
   delay(100);
	 ...
	 indice2= waitResponse(60500L,"0,0","0,1","0,2");
	 if(indice2!=1)
		{
			switch(indice2)
			{
	 ...
	 indice3= waitResponse(10000L,",0" GSM_NL,",1" GSM_NL,",2" GSM_NL,",3" GSM_NL,",4" GSM_NL);
