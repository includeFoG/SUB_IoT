...
public:
   ...
   */
     ...
     // GPS ON
  uint8_t enableGPS(uint8_t mode=1, uint8_t aidMode=0, uint8_t GSNSSSystem=1, bool uggga=true, bool uggll=true, bool time = true) {  
	String resp="";
	uint8_t ind=0;
 ...
 // Get GNSS fix data
  String getGPS_NMEA() {
	...
	if(waitResponse(15000L)!=1){ // espera el OK
		return "Err2";//ERROR 2: EL MODULO NO DEVUELVE OK
	}
 ...
 // } 
  ...
  String satellitesViewGPS(){
   ...
   uint8_t ind=0;
	...
	resp = stream.readStringUntil('OK');
	if(waitResponse()!=1){
		return "3"; //ERROR 3:
    }
