 ...
 delay(3000); 
	...
	int res1 = waitResponse(10000L,resp,GF("OK"));
		if (res1 != 1) {
			...
			DBG("SaraR4 en funcionalidad mínima");
...
bool F_name=false;
    ...
    if (waitResponse(10000L) != 1) {
      return false;
		...
		sendAT(GF("+COPS=1,2,21407"));        // Manual-register to the network
    ...
    if (waitResponse(10000L) != 1) {
      return false;
		...
		F_name=false;
		sendAT(GF("+UMQTT=0,\"100000002\""));      //cliente      
    if (waitResponse(10000L) != 1) {
      return false;
    }
	}
...
TINY_GSM_MODEM_WAIT_FOR_NETWORK()
 ...
 delay(3000); //añadido
	...
	int res1 = waitResponse(60000L,resp,GF("OK"), GF("+CME")); //200000L
		if (res1 != 1) {
			waitResponse();
			if(res1==2){
			...
			DBG("SaraR4 desconectado del servicio GPRS");
