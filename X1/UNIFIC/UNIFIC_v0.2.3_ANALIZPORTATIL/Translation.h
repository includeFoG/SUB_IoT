...
******* Transaltion tables ****************************************/
  ...
  const char TEXT_RISING_FAST[]       = "0.1";//"en rápido aumento";
  const char TEXT_RISING[]            = "0.2";//"en aumento";
  const char TEXT_RISING_SLOW[]       = "0.3";//"en aumento lento";
  const char TEXT_STEADY[]            = "0.4";//"estable";
  const char TEXT_FALLING_SLOW[]      = "0.5";//"en lento descenso";
  const char TEXT_FALLING[]           = "0.6";//"en descenso";
  const char TEXT_FALLING_FAST[]      = "0.7";//"en rápido descenso";

  const char TEXT_ZAMBRETTI_FORECAST[]= "Pronóstico";
  const char TEXT_ZAMBRETTI_ACCURACY[]= "Precisión de pronóstico";
  const char TEXT_ZAMBRETTI_A[]       = "1";//"Clima estable";
  const char TEXT_ZAMBRETTI_B[]       = "2";//"Buen clima";
  const char TEXT_ZAMBRETTI_C[]       = "3";//"Mejorando";
  const char TEXT_ZAMBRETTI_D[]       = "4";//"Buen clima, pero inestable";
  const char TEXT_ZAMBRETTI_E[]       = "5";//"Posibles lluvias si hay nubes";
  const char TEXT_ZAMBRETTI_F[]       = "6";//"Muy buen clima, mejorando";
  const char TEXT_ZAMBRETTI_G[]       = "7";//"Buen clima pero con posibles lloviznas si hay nubes";
  const char TEXT_ZAMBRETTI_H[]       = "8";//"Buen clima pero con probabilidad de lloviznas si hay nubes";
  const char TEXT_ZAMBRETTI_I[]       = "9";//"Alta probabilidad de llovizna si hay nubes, clima mejorando";
  const char TEXT_ZAMBRETTI_J[]       = "10";//"Inestable con probabilidad de mejora";
  const char TEXT_ZAMBRETTI_K[]       = "11";//"Buen clima con probables lluvias si hay nubes";
  const char TEXT_ZAMBRETTI_L[]       = "12";//"Inestable, se espera mejora";
  const char TEXT_ZAMBRETTI_M[]       = "13";//"Clima inestable con probables mejoras";
  const char TEXT_ZAMBRETTI_N[]       = "14";//"Posibles intervalos lluviosos si hay nubes";
  const char TEXT_ZAMBRETTI_O[]       = "15";//"Alta probabilidad de lluvia si hay nubes, se vuelve inestable";
  const char TEXT_ZAMBRETTI_P[]       = "16";//"Inestable, alta probabilidad de lluvia si hay nubes";
  const char TEXT_ZAMBRETTI_Q[]       = "17";//"Inestable con intervalos de buen tiempo";
  const char TEXT_ZAMBRETTI_R[]       = "18";//"Inestable, se esperan lluvias";
  const char TEXT_ZAMBRETTI_S[]       = "19";//"Inestable, lluvias ocasionales si hay nubes";
  const char TEXT_ZAMBRETTI_T[]       = "20";//"Muy inestable";
  const char TEXT_ZAMBRETTI_U[]       = "21";//"Lluvias ocasionales si hay nubes, previsión: empeora";
  const char TEXT_ZAMBRETTI_V[]       = "22";//"Lluvia ocasional si hay nubes con fuerte inestabilidad";
  const char TEXT_ZAMBRETTI_W[]       = "23";//"Precipitaciones frecuentes si hay nubes";
  const char TEXT_ZAMBRETTI_X[]       = "24";//"Muy inestable, lluvia si hay nubes";
  const char TEXT_ZAMBRETTI_Y[]       = "25";//"Tormenta con posibles mejoras";
  const char TEXT_ZAMBRETTI_Z[]       = "26";//"Tormenta, mucha lluvia";
  const char TEXT_ZAMBRETTI_0[]       = "27";//"¡Batería baja, por favor recargue!";
  const char TEXT_ZAMBRETTI_DEFAULT[] = "28";//"Sin pronóstico por el momento";
