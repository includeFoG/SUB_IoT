...
String buildGenJSON_2() {

  Serial.println("buildGenJSON_2");

  //  DynamicJsonDocument doc(1024);
  DynamicJsonDocument _JSON(1024);
  _JSON["xh"] = "SSM-whst-009";

  //          JsonArray newRegisters = newDoc.createNestedArray("registers");

  // Obtener los dispositivos del JSON original
  ...
  int counter = 0;
  // Recorrer cada dispositivo y agregar sus registros al nuevo JSON
...
//    Serial.println("aki_1");
    for (JsonObject reg : registers) {
      ...
      float valor = reg["valor"].as<float>();
      valor = round(valor * 100) / 100.0;
      _JSON[name] = String(valor);  // Guardar en el JSON con su nombre
      //              Serial.print("Añadiendo: "); Serial.print(name);
      //              Serial.print(" = "); Serial.println(valor);
    }
  }
  // Mostrar el nuevo JSON

  if (_JSON.isNull()) {
    Serial.println("Error: JSON no se creó correctamente");
    return "";
  }

  String aux = "";
  serializeJson(_JSON, aux);

  return aux;

  /*
    StaticJsonDocument<1024> newDoc;
    newDoc["xh"] = "SSM-whst-009";
    ...
    String aux;
    Serial.print("aki_6");
    serializeJson(newDoc, aux);  // si haces esto, el programa peta..........  Xavi mira esto
    serializeJson(newDoc, Serial);  // si haces esto, compila y ejecuta el codigo sin problema.

    Serial.print("este ese el aux");
    Serial.println(aux);
  ...
  */
  ...
  /*
    ...
    Serial.println("aki3");
    Serial.print("NUMITEMS(s_ValJSON) = ");
    Serial.println(NUMITEMS(s_ValJSON));
...
int EnviaMQTT_2(String MessageToSend)
{
  Serial.println("EnviaMQTT");
  ...
  Serial.print("Topic = "); Serial.println(finalTopic);
...
int EnviaMIKROE_2(String MessageToSend)
{
//  serializeJson(_JSON, MessageToSend);
  Serial.println("EnviaMIKROE_2");
...
int envioDatosRS485Muutech_2(String JSONENVIO) {

  Serial.println("envioDatosRS485Muutech_2");
  ...
  if (a_sendTime) {
    ...
    if (modem.isNetworkConnected()) {
      retorno = EnviaMIKROE_2(JSONENVIO);
      return retorno;
...
int envioDatosRS485Muutech(String JSONENVIO) {
  ...
  Serial.println("envioDatosRS485Muutech");
  current_timestamp = rtc.getEpoch();
  sum_SecAct = ((hour(current_timestamp) * 3600) + (minute(current_timestamp) * 60) + second(current_timestamp));
