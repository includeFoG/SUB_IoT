...
shrPrta::shrPrta( byte address, byte width, TwoWire *awire_)
		   : i2caddr (address), i2cwidth (width),wire_(awire_), iodir_state (0xffff){}
		   
		   
//Obtiene la instancia actual de shrPrta
shrPrta& shrPrta::getInstance(byte address, byte width, TwoWire* awire_)
{
	static shrPrta instance(address, width, awire_); // Instancia estática
	return instance;
}
