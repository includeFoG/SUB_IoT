...
// 
// ***********************************************************************
//  ESTA CLASE SE HA MODIFICADO PARA CREAR UN PATRON SINGLETON 
//  V.0.0.0
//  FoG
// ***********************************************************************
...
public:
	...
	static shrPrta& getInstance(byte address, byte width = 1, TwoWire *awire_ = &Wire ); //AÑADIDO	
			   
	/*shrPrta( byte address, byte width = 1, TwoWire *awire_ = &Wire ) 
		   { i2caddr = address; i2cwidth = width; iodir_state = 0xffff; wire_=awire_; }	*/
	
	shrPrta(const shrPrta&) = delete; 				//eliminamos el constructor de copia
	shrPrta& operator = (const shrPrta&) = delete;  //eliminamos asignación por defecto del compilador
...
private:
	shrPrta( byte address, byte width = 1, TwoWire *awire_ = &Wire ); //AÑADIDO
